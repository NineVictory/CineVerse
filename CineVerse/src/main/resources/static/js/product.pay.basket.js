$(function() {
    ...
    function updateHiddenInput(value, mc_num) {
        let deliveryFee = value >= 50000 ? 0 : 3000;
        $('input[name="total"]').val(`${value + deliveryFee}`); 
    ...
    $('.realBuy').on('click', function(event) {
        ...
        const total = document.querySelector('.total_price .tt').textContent.replace('원', '').replace(/,/g, ''); // 콤마 제거
    
        $('#hiddenTotalCount').val(total_count);
        $('#hiddenTotal').val(total);
        $('#hiddenCount').val(count - 1);
