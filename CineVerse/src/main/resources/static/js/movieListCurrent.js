...
/*	// 초기 영화 목록 설정
    ...
    $("#statusCheckbox").change(function() {
       var status = $(this).is(':checked') ? 1 : '';
        location.href = '/movie/movieListCurrent?status=' + status;
...
/*        currentPage = 1;
   ...
   // 최신순, 북마크순 체크박스 상태 관리
    $("input[name='movieorder']").change(function() {
        ...
        location.href = '/movie/movieListCurrent?keyfield=' + $('#keyfield-select').val() + '&keyword=' + $('#keyword').val() + '&movieorder=' + movieorder;
    });
    ...
    function loadMoreMovies() {
        ...
        $.ajax({
            ...
            success: function(response) {
                const movieListCurrent = response.movieListCurrent;

                if (movieListCurrent.length > 0) {
                    let html = '';
                    movieListCurrent.forEach(movie => {
                ...
                if (movieListCurrent.length < rowCount) {
                    $("#loadMoreButton").hide();
                }
            },
