<%@ page language="java" contentType="text/html; charset=UTF-8"
	pageEncoding="UTF-8"%>
	<link rel="stylesheet" href="${pageContext.request.contextPath}/css/footer.css" type="text/css">
<!-- 하단 시작 -->
<footer class="footer">
	<div class="footer-content">
		<div class="footer-main-logo">
			<a href="${pageContext.request.contextPath}/main/main">
			<img src="${pageContext.request.contextPath}/images/cmj/logo.png"
				width="160">
			</a>
			
		</div>
		<div class="footer-menu">
			<ul class="f_menu type1">
				<li><a href="${pageContext.request.contextPath}/main/main">메인 </a></li>
				<li><a href="${pageContext.request.contextPath}/myPage/myPageMain">마이페이지 </a></li>
				<li><a href="${pageContext.request.contextPath}/movie/movieList">영화 목록 </a></li>
				<li><a href="${pageContext.request.contextPath}/movie/movieReserve">영화 예매 </a></li>
				<li><a href="${pageContext.request.contextPath}/board/list">커뮤니티 </a></li>
				<li><a href="${pageContext.request.contextPath}/assignboard/list">양도/교환 </a></li>
				<li><a href="${pageContext.request.contextPath}/support/main">고객센터 </a></li>
				<li><a href="${pageContext.request.contextPath}/member/pointCharge">포인트 </a></li>
				<li><a href="${pageContext.request.contextPath}/shop/shopMain">벌스샵</a></li>
			</ul>
		</div>
		<div class="footer-company">
			<ul>
				<li>서울특별시 용산구 한강대로 23길 55, 아이파크몰 6층(한강로동)</li>
				<li>구승회@NineVictory 함종태@HamJongtae 김보미@kxxbombom 박근휘@geunhwi98 최주은@KimGryut 박민재@boonmojae 조민지@minji0626</li>
				<li>호스팅사업자 : CV쌍용네트웍스 개인정보보호 책임자 : 분모재 대표이메일 : movie.cineverse@gmail.com</li>
				<li>&copy; CV CINEVERSE. All Rights Reserved</li>
			</ul>
		</div>
	</div>

</footer>
<!-- 하단 끝 -->