<%@ page language="java" contentType="text/html; charset=UTF-8"
	pageEncoding="UTF-8"%>
<!--  내가 본 영화 시작 -->
<div class="myPage_main">

	<div class="myPage_title">내가 본 영화</div>
	<hr size="1" width="100%" class="wa_line">


	<div class="wa_box_rep">
		<div class="wa_box">
			<div class="wa_photo">
				<img src="${pagecontext.reuqest.contextPath }/images/pmj/dog.jpg"
					alt="내가 본 영화1" width="150" height="210">
			</div>
			<div class="wa_movie_info">
				<div class="wa_info_name">듄</div>
				<div class="wa_info_sub">CGV 용산아이파크몰</div>
				<div class="wa_info_sub">2024.07.06(토) 26:00</div>
			</div>
		</div>
		<hr size="1" width="100%" class="wa_line">
	</div>



	<div class="wa_box_rep">
		<div class="wa_box">
			<div class="wa_photo">
				<img src="${pagecontext.reuqest.contextPath }/images/pmj/dune.jpg"
					alt="내가 본 영화2" width="150" height="210">
			</div>
			<div class="wa_movie_info">
				<div class="wa_info_name">듄</div>
				<div class="wa_info_sub">CGV 용산아이파크몰</div>
				<div class="wa_info_sub">2024.07.06(토) 26:00</div>
			</div>
		</div>
		<hr size="1" width="100%" class="wa_line">
	</div>




</div>
<!--  내가 본 영화 끝 -->