				...
				<tbody>
					<c:forEach var="event" items="${list}">
						<tr>
							...
							<td class="mem-data"
								onclick="redirectToEventDetail(${event.event_num})"
								id="intoEventDetail">${event.event_name}</td>
							<td class="mem-data">${event.ep_date}</td>
							<td class="mem-data">${event.mem_id}</td>
							<td class="mem-data"><c:if
									test="${event.ep_content == 'crowd_movie1'}">${event.crowd_movie1}</c:if>
