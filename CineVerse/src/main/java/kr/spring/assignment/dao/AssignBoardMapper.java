...
public interface AssignBoardMapper {
	 ...
	 * public Integer selectRowCountReply(Map<String,Object> map); //댓글 수정,삭제시 작성자
	 * 회원번호를 구하기 위해 사용
	 * 
	 * @Select("SELECT * FROM community_comment WHERE re_num=#{cc_num}") public
	 * BoardReplyVO selectReply(Long cc_num); public void insertReply(BoardReplyVO
	 * boardReply); //cc_ip 컬럼 추가할 것
	 * 
	 * @Update("UPDATE spboard_reply SET cc_content=#{cc_content},cc_ip=#{cc_ip},cc_modify_date=SYSDATE WHERE cc_num=#{cc_num}"
	 * ) public void updateReply(BoardReplyVO boardReply);
	 * 
	 * @Delete("DELETE FROM community_comment WHERE cc_num=#{cc_num}") public void
	 * deleteReply(Long cc_num); //부모글 삭제시 댓글이 존재하면 부모글 삭제전 댓글 삭제
	 * 
	 * @Delete("DELETE FROM community_comment WHERE cb_num=#{cb_num}") public void
	 * deleteReplyByBoardNum(Long cb_num); //부모글 삭제시 댓글의 답글이 존재하면 댓글 번호를 구해서 답글 삭제시
	 * 사용
	 * 
	 * @Select("SELECT * FROM community_comment WHERE cb_num=#{cb_num}") public
	 * List<Long> selectReNumsByBoard_num(Long cb_num);
	 * 
	 * 
	 * //댓글 좋아요
	 * 
	 ...
	 * 
	 * @Select("SELECT COUNT(*) FROM community_fav WHERE cc_num=#{cc_num}") public
	 * Integer selectReFavCount(Long re_num);
	 * 
	 ...
	 * 
	 * @Delete("DELETE FROM community_fav WHERE cc_num=#{cc_num}") public void
	 * deleteReFavByReNum(Long cc_num);
	 * 
